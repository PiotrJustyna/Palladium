...
docker build -t silo-host-cluster -f ./ops/Dockerfile ./ &&
  docker run -it -e ADVERTISEDIP=$ADVERTISEDIP  -e GATEWAYPORT=$GATEWAYPORT -e SILOPORT=$SILOPORT -e PRIMARYPORT=$PRIMARYPORT -e DASHBOARDPORT=$DASHBOARDPORT\
   -e PRIMARYADDRESS=$PRIMARYADDRESS -e SMOKETESTSAPIPORT=$SMOKETESTSAPIPORT -p $GATEWAYPORT:$GATEWAYPORT -p $SILOPORT:$SILOPORT -p $DASHBOARDPORT:$DASHBOARDPORT -p $SMOKETESTSAPIPORT:$SMOKETESTSAPIPORT --rm silo-host-cluster